    ...
    <div className="auth-page">
      <div className="auth-card animate-scale-in">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-lg)' }}>
          <Input type="email" label="Email Address" placeholder="you@example.com" value={email} onChange={setEmail} error={error && !email ? 'Required' : ''} />
          <Input type="text" label="Password" placeholder="••••••••" value={password} onChange={setPassword} error={error && !password ? 'Required' : ''} />
        </div>

        <div style={{ textAlign: 'right', marginTop: 'var(--space-sm)' }}>
          <a href="/auth/reset-password" style={{ fontSize: '0.78rem', color: 'var(--color-accent)' }}>Forgot password?</a>
        </div>

        {error && <p style={{ color: 'var(--color-error)', fontSize: '0.8rem', marginTop: 'var(--space-md)' }}>{error}</p>}

        <Button title="Sign In" action={handleSignIn} variant={loading ? 'disabled' : 'solid'} size="lg" loading={loading} classes="auth-submit-btn" />

        <p style={{ textAlign: 'center', fontSize: '0.82rem', color: 'var(--color-text-muted)', marginTop: 'var(--space-lg)' }}>
          Don't have an account? <a href="/auth/signup" style={{ color: 'var(--color-primary)', fontWeight: 600 }}>Sign Up</a>
        </p>
      </div>
      ...
      <style>{`
        ...
        .auth-google-btn { display: flex; align-items: center; justify-content: center; gap: var(--space-md); width: 100%; padding: 0.75rem; border: 1.5px solid var(--color-border); border-radius: var(--radius-md); font-size: 0.85rem; font-weight: 500; background: white; transition: background var(--transition-fast), border-color var(--transition-fast); margin-bottom: var(--space-lg); }
        .auth-google-btn:hover { background: var(--color-surface); border-color: var(--color-text-muted); }

        ...
        {MOCK_ORDERS.map((order) => (
          <div key={order.id} style={{ background: 'white', borderRadius: 'var(--radius-lg)', border: '1px solid var(--color-border)', padding: 'var(--space-xl)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 'var(--space-md)' }}>
              <div>
                <p style={{ fontWeight: 700, fontFamily: 'var(--font-mono)', marginBottom: '0.25rem' }}>{order.id}</p>
                <p style={{ fontSize: '0.78rem', color: 'var(--color-text-muted)' }}>{new Date(order.date).toLocaleDateString('en-NG', { day: 'numeric', month: 'long', year: 'numeric' })} · {order.items} item{order.items > 1 ? 's' : ''}</p>
              </div>
